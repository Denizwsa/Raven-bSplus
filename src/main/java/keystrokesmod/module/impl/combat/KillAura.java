...
public class KillAura extends Module {
    ...
    private ButtonSetting fakeAutoblock;
    private SliderSetting autoBlockMode;
    private SliderSetting polarMode;
    private SliderSetting polarSpeed;
    private ButtonSetting realAutoblock;
    ...
    public KillAura() {
        ...
        this.registerSetting(fov = new SliderSetting("FOV", "°", 180.0, 30.0, 360.0, 4.0));
        ...
        this.registerSetting(aimRange = new SliderSetting("Range (aim)", 3.3, 3.0, 8.0, 0.05));
        ...
        this.registerSetting(fakeAutoblock = new ButtonSetting("Fake Autoblock", false));
        this.registerSetting(autoBlockMode = new SliderSetting("AutoBlock", 0,
                new String[]{"Disabled", "Pre-attack", "Post-attack", "Both"}));
        this.registerSetting(realAutoblock = new ButtonSetting("Real AutoBlock (block on hit)", false));
        this.registerSetting(polarMode = new SliderSetting("Polar mode", 0, new String[]{"Normal", "Polar"}));
        this.registerSetting(polarSpeed = new SliderSetting("Polar speed", 5, 1, 10, 1));
    }

    @Override
    public void guiUpdate() {
        boolean polar = polarMode.getInput() == 1;
        polarSpeed.setVisible(polar, this);
    }
    ...
    public void onRenderTick(TickEvent.RenderTickEvent e) {
        ...
        if (!fakeAutoblock.isToggled()) return;
        if (!Utils.nullCheck() || mc.currentScreen != null) {
            ReflectionUtils.setItemInUse(false);
            return;
        }
        // Autoblock modülü zaten aktifse çakışmayı önle (şu an autoBlock modülü yok)
        boolean shouldBlock = target != null
                && targetDistance <= attackRange.getInput()
                && Utils.holdingSword()
                && basicCondition();
        ReflectionUtils.setItemInUse(shouldBlock);
    }
    ...
    public void onClientRotation(ClientRotationEvent e) {
        ...
        if (rotationMode.getInput() == 0) {
            double aimRangeVal = polar ? Math.min(3.05, aimRange.getInput()) : aimRange.getInput();
            if (targetDistance <= aimRangeVal) {
                int speedVal = polar ? (int) polarSpeed.getInput() : (int) speed.getInput();
                boolean useBackup = !aimThroughBlocks.isToggled() || !aimThroughEntities.isToggled();
                float[] rot = RotationHelper.get().getRotationsToTarget(target, e, speedVal, 100, 100, 0f, useBackup, aimRangeVal, polar ? false : aimThroughBlocks.isToggled(), aimThroughEntities.isToggled());
                if (rot != null) {
                    float jitterVal = (float) jitter.getInput();
                    if (jitterVal > 0) {
                        rot[0] += (rand.nextFloat() - 0.5f) * jitterVal;
                        rot[1] += (rand.nextFloat() - 0.5f) * jitterVal;
                    }
                    if (polar) {
                        rot[0] += (rand.nextFloat() - 0.5f) * 1.5f;
                        rot[1] += (rand.nextFloat() - 0.5f) * 1.0f;
                    }
                    rot[1] = Math.max(-90f, Math.min(90f, rot[1]));
                    rot[0] = (rot[0] % 360f + 360f) % 360f;
                    e.yaw = rot[0];
                    e.pitch = rot[1];
                }
            }
    ...
    public void onUpdate() {
        ...
        if (rotationMode.getInput() == 1 && target != null) {
            ...
            if (targetDistance <= aimRangeVal) {
                int speedVal = polar ? (int) polarSpeed.getInput() : (int) speed.getInput();
                boolean useBackup = !aimThroughBlocks.isToggled() || !aimThroughEntities.isToggled();
                float[] rot = RotationHelper.get().getRotationsToTarget(target, speedVal, 100, 100, 0f, useBackup, aimRangeVal, polar ? false : aimThroughBlocks.isToggled(), aimThroughEntities.isToggled());
                if (rot != null) {
    ...
    public void onPrePlayerInteract(PrePlayerInteractEvent e) {
        ...
        if (targetDistance > swingRange.getInput()) return;
         ...
         // Handle AutoBlock
         int blockMode = (int) autoBlockMode.getInput();
         if (blockMode > 0 && Utils.holdingSword() && target != null) {
             // Pre-attack or Both mode
        ...
        if (polarMode.getInput() == 1 && clicks > 0 && rand.nextInt(4) == 0) {
            clicks--;
        }
        ...
        // Post-attack or Both mode - block after hitting
        if ((blockMode == 2 || blockMode == 3) && Utils.holdingSword()) {
            KeyBinding.setKeyBindState(mc.gameSettings.keyBindUseItem.getKeyCode(), true);
        }
    }
    ...
    private void handleTarget() {
        ...
        if (polar) {
            maxRange = Math.min(3.05, maxRange);
        }
    ...
    private Candidate getCandidateTarget(Entity entity, double maxRange, float fovValue) {
        ...
        if (polarMode.getInput() == 1 && !mc.thePlayer.canEntityBeSeen(entity)) {
            return null;
        }
    ...
    private KillAuraTarget buildKillAuraTarget(EntityLivingBase entity, double distanceToBoundingBox, double maxRange) {
        ...
        boolean polar = polarMode.getInput() == 1;
        boolean allowThroughBlocks = polar ? false : aimThroughBlocks.isToggled();
        double multipointH = 100;
        double multipointV = 100;
        if (!RotationUtils.hasValidAimPoint(entity, multipointH, multipointV, maxRange, allowThroughBlocks, aimThroughEntities.isToggled())) {
            return null;
        }
    ...
    private long nextDelay() {
        ...
        // Gaussian distribution for more human-like clicking
        double val = rand.nextGaussian() * (baseDelay * 0.2); // 20% standard deviation
        int finalDelay = (int) (baseDelay + val);
        
        return Math.max(33, Math.min(250, finalDelay));
    }
    ...
    public void modifyMouseOverFromGetMouseOver(float partialTicks) {
        ...
        // Use intercept point if available, otherwise use eye position (player is inside entity)
        Vec3 hitVec;
        if (intercept != null) {
            hitVec = intercept.hitVec;
        } else {
            // Player is inside entity - use closest point on bounding box
            hitVec = getClosestPointOnAABB(eyes, bb);
        }
    ...
    /**
     * Gets closest point on AABB to a given point
     */
    private Vec3 getClosestPointOnAABB(Vec3 point, AxisAlignedBB aabb) {
        double closestX = Math.max(aabb.minX, Math.min(point.xCoord, aabb.maxX));
        double closestY = Math.max(aabb.minY, Math.min(point.yCoord, aabb.maxY));
        double closestZ = Math.max(aabb.minZ, Math.min(point.zCoord, aabb.maxZ));
