...
public class BedAura extends Module {
    ...
    private final SliderSetting grimRotationSpeed;
    ...
    private static final double PATH_STEP = 0.3;
    ...
    private BlockPos[] targetBedPair;
    ...
    public BedAura() {
        ...
        this.registerSetting(mode = new SliderSetting("Mode", 0, new String[]{"Legit", "Grim"}));
        ...
        this.registerSetting(grimRotationSpeed = new SliderSetting("Grim rotation speed", 3, 1, 10, 1));
    }

    @Override
    public void guiUpdate() {
        outlineColor.setVisible(renderOutline.isToggled(), this);
        boolean grim = mode.getInput() == 1;
        grimRotationSpeed.setVisible(grim, this);
    }
    ...
    public float getBreakSpeedMultiplier() {
        ...
        float result = multiplier > 1.0f ? multiplier : 1.0f;
        if (mode.getInput() == 1) {
            result = Math.min(1.05f, result);
        }
        return result;
    }
    ...
    public void onClientRotation(ClientRotationEvent e) {
        ...
        BlockPos[] bestPair = findClosestBedPair();
        if (bestPair == null) {
            resetMining();
            return;
        }

        if (targetPos != null && BlockUtils.getBlock(targetPos) == Blocks.air) {
            pathIndex++;
            targetPos = null;
            targetHitVec = null;
            targetSide = null;
        ...
        boolean exposed = isBedExposed(bestPair, null);
        if (!exposed && pathBlocks.isEmpty()) {
            pathBlocks = computePathToBed(bestPair);
            pathIndex = 0;
        }

        if (targetBedPair != bestPair) {
            targetBedPair = bestPair;
            pathBlocks = computePathToBed(bestPair);
            pathIndex = 0;
        }

        if (!exposed && !pathBlocks.isEmpty() && pathIndex < pathBlocks.size()) {
            BlockPos next = pathBlocks.get(pathIndex);
            if (BlockUtils.getBlock(next) == Blocks.air) {
                pathIndex++;
                if (pathIndex < pathBlocks.size()) {
                    next = pathBlocks.get(pathIndex);
                } else {
                    exposed = true;
                }
            }
            if (!exposed) {
                Choice ch = blockToChoice(next, reachSq);
                if (ch != null) {
                    targetPos = ch.pos;
                    targetHitVec = ch.hitVec;
                    targetSide = ch.side;
                    miningActive = true;
                    equipBestHotbarTool(BlockUtils.getBlock(targetPos));
                    applyRotation(e, targetPos, targetHitVec);
                    return;
                }
            }
        }
    ...
    private void applyRotation(ClientRotationEvent e, BlockPos pos, Vec3 hitVec) {
        ...
        if (mode.getInput() == 1) {
            ...
            float speed = (float) grimRotationSpeed.getInput();
            r = RotationUtils.smoothRotation(baseYaw, basePitch, r[0], r[1], (int) speed, 0f);
        }
        e.setYaw(r[0]);
        e.setPitch(r[1]);
    }
    ...
    private List<BlockPos> computePathToBed(BlockPos[] bedPair) {
        Vec3 start = mc.thePlayer.getPositionEyes(1.0f);
        Vec3 end = bedCenter(bedPair);
        ...
        if (dist < 0.1) return result;

        int steps = Math.max(1, (int) (dist / PATH_STEP));
        Set<BlockPos> bedSet = new HashSet<>();
        bedSet.add(bedPair[0]);
        bedSet.add(bedPair[1]);
        ...
        for (int i = 1; i <= steps; i++) {
            ...
            if (bedSet.contains(pos)) break;
            if (seen.contains(pos)) continue;

            Block block = BlockUtils.getBlock(pos);
            if (block == Blocks.air || block instanceof BlockBed) continue;
    ...
    private Choice blockToChoice(BlockPos pos, double reachSq) {
        Block block = BlockUtils.getBlock(pos);
        ...
        if (eye.squareDistanceTo(hit) > reachSq + 1e-3) return null;

        EnumFacing side = BlockUtils.facingFromBlockCenterToPoint(pos, hit);
        return new Choice(pos, hit, side);
    }

    private BlockPos[] findClosestBedPair() {
        sortBedsByEyeDistance(bedPairsCache);
        return bedPairsCache.isEmpty() ? null : bedPairsCache.get(0);
    }
        ...
        for (int dx = -ri; dx <= ri; dx++) {
            for (int dy = -ri; dy <= ri; dy++) {
                for (int dz = -ri; dz <= ri; dz++) {
                    ...
                    if (seenFeet.contains(foot)) continue;
                    if (!bedInSearchRange(pair, searchRange)) continue;
                    Vec3 center = bedCenter(pair);
                    if (!inFov(center, (float) fov.getInput())) continue;
                    seenFeet.add(foot);
    ...
    private BlockPos[] footHeadPair(BlockPos at) {
        ...
        if (!(st.getBlock() instanceof BlockBed)) return null;
        ...
        if (!(footSt.getBlock() instanceof BlockBed)) return null;
        if (footSt.getValue(BlockBed.PART) != BlockBed.EnumPartType.FOOT) return null;
        ...
        if (!(hs.getBlock() instanceof BlockBed)) return null;
        if (hs.getValue(BlockBed.PART) != BlockBed.EnumPartType.HEAD) return null;
        if (hs.getValue(BlockBed.FACING) != footFacing) return null;
    ...
    private boolean bedInSearchRange(BlockPos[] pair, double searchRadius) {
        ...
        if (eye.squareDistanceTo(onBox) <= r2) return true;
    ...
    private boolean isBedExposed(BlockPos[] pair, Set<BlockPos> pathBlocks) {
        for (BlockPos bp : pair) {
            for (EnumFacing f : EnumFacing.values()) {
                BlockPos n = bp.offset(f);
                IBlockState state = mc.theWorld.getBlockState(n);
                if (state.getBlock() == Blocks.air) return true;
                if (state.getBlock().isReplaceable(mc.theWorld, n)) return true;
                if (pathBlocks != null && pathBlocks.contains(n)) return true;
            }
        }
        return false;
    }
    ...
    private double scoreChoice(Choice ch, float curProg, BlockPos breaking) {
        ...
        if (bestHotbar <= 0) return Double.POSITIVE_INFINITY;
    ...
    private List<Choice> buildCandidates(BlockPos[] pair, double reachSq) {
        ...
        boolean exposed = isBedExposed(pair, null);
    ...
    private void addBlockCandidate(BlockPos pos, double reachSq, List<Choice> out) {
        ...
        Vec3 eye = mc.thePlayer.getPositionEyes(1.0f);
        Vec3 hit = RotationUtils.closestPointOnAabb(bb, eye);
        if (eye.squareDistanceTo(hit) > reachSq + 1e-3) return;
        ...
        if (block instanceof BlockBed && side == EnumFacing.DOWN) return;
    ...
    private boolean shouldYieldToKillAura() {
        if (!prioritizeKillAura.isToggled()) return false;
    ...
    private void removeOwnBedPair() {
        if (!shouldWhitelistOwnBed() || bedPairsCache.isEmpty()) return;
