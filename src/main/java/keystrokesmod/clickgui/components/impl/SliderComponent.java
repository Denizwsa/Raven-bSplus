...
public class SliderComponent extends Component {
    ...
    private static final Color TRACK_BG = new Color(30, 30, 50, 180);
    private static final Color FILL_START = new Color(60, 180, 255);
    private static final Color FILL_END = new Color(100, 80, 255);
    ...
    public SliderComponent(SliderSetting sliderSetting, ModuleComponent moduleComponent, float o) {
        ...
        this.width = this.sliderSetting.getInput() == -1 ? 0
            : (double) (this.moduleComponent.categoryComponent.getWidth() - 10)
    ...
    public void render() {
        ...
        float catW = this.moduleComponent.categoryComponent.getWidth();
        float renderY = this.moduleComponent.categoryComponent.getY() + this.o;

        float trackLeft = catX + 5 + (xOffset / 2);
        float trackRight = catX + 5 + catW - 10;
        float trackTop = renderY + 11;
        float trackBottom = renderY + 14;

        RenderUtils.drawRoundedRectangle(trackLeft, trackTop, trackRight, trackBottom, 2, TRACK_BG.getRGB());

        float fillLeft = trackLeft;
        float fillRight = (float) Math.min(trackLeft + this.width, trackRight);
        if (fillRight - fillLeft > 2) {
            RenderUtils.drawHorizontalGradientRect(fillLeft, trackTop, fillRight, trackBottom,
                    FILL_START.getRGB(), FILL_END.getRGB());
        }
        ...
        float labelX = (catX + 5) * 2 + xOffset;
        float labelY = (renderY + 3) * 2;
    ...
    public void drawScreen(int mouseX, int mouseY) {
        ...
        if (this.heldDown) {
            double d = Math.min(this.moduleComponent.categoryComponent.getWidth() - 10, Math.max(0, mouseX - this.x - 5));
            if (d == 0.0 && this.sliderSetting.canBeDisabled) {
                this.targetValue = -1;
            }
            else {
                double n = roundToInterval(
                    d / (double) (this.moduleComponent.categoryComponent.getWidth() - 10)
            ...
            if (this.displayedValue == -1) {
                ...
                this.width = (this.moduleComponent.categoryComponent.getWidth() - 10) * fraction;
            }
