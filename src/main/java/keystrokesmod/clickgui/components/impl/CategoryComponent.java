...
public class CategoryComponent {
    ...
    public void onScroll(int mouseScrollInput, float mouseX, float mouseY) {
        ...
        if (!Float.isNaN(mouseX) && !Float.isNaN(mouseY)) {
            ...
            for (ModuleComponent mod : this.modules) {
                if (clickGui != null && !clickGui.matchesSearch(mod)) {
                    continue;
                }
    ...
    /** Arama aktifken kapalı kategorilerin içeriğini de gösterir. */
    public void renderWithSearch(FontRenderer renderer, boolean searchActive) {
        if (searchActive && !this.opened) {
            // Arama modunda: geçici olarak açık gibi davran ama animasyonsuz
            // Önce normal başlığı çiz (kapalı görünümde)
            render(renderer);
            // Eşleşen modüller varsa kategorinin altında render et
            ClickGui clickGui = Minecraft.getMinecraft().currentScreen instanceof ClickGui ? (ClickGui) Minecraft.getMinecraft().currentScreen : null;
            if (clickGui == null) return;
            boolean hasMatch = this.modules.stream().anyMatch(m -> clickGui.matchesSearch(m));
            if (!hasMatch) return;

            // Eşleşen modülleri başlığın altında çiz
            float drawY = this.y + this.titleHeight + 4;
            float totalH = 0f;
            for (ModuleComponent c : this.modules) {
            ...
            RenderUtils.drawRoundedGradientOutlinedRectangle(this.x - 2, this.y + this.titleHeight + 2,
                    this.x + this.width + 2, this.y + this.titleHeight + 4 + totalH + 2,
            ...
            // drawY: kategorinin üst kenarından itibaren offset
            float relY = this.titleHeight + 4; // başlığın altından başla
            for (ModuleComponent c : this.modules) {
    ...
    public void render(FontRenderer renderer) {
        ...
        if (this.opened || smoothTimer != null) {
            ...
            for (ModuleComponent c2 : this.modules) {
                if (clickGui != null && !clickGui.matchesSearch(c2)) {
                    continue;
                }
                c2.render();
            }
    ...
    public void updateHeight() {
        ...
        for (ModuleComponent component : this.modules) {
            if (clickGui != null && !clickGui.matchesSearch(component)) {
                continue;
            }
