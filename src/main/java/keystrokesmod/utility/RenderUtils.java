...
public class RenderUtils implements IMinecraftInstance {
    ...
    public static void drawGradientRect(int x, int y, int w, int h, int color1, int color2, int alpha) {
        float a1 = Math.min(1f, (color1 >> 24 & 0xFF) / 255f) * (alpha / 255f);
        float r1 = (color1 >> 16 & 0xFF) / 255f;
        float g1 = (color1 >> 8 & 0xFF) / 255f;
        float b1 = (color1 & 0xFF) / 255f;
        float a2 = Math.min(1f, (color2 >> 24 & 0xFF) / 255f) * (alpha / 255f);
    ...
    public static void drawRoundedGradientRectangle(float x, float y, float x2, float y2, float radius, int topColor, int bottomColor) {
        x *= 2.0f; y *= 2.0f; x2 *= 2.0f; y2 *= 2.0f;
        GL11.glPushMatrix();
        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
        GL11.glScaled(0.5, 0.5, 0.5);
        glEnable(3042); GL11.glDisable(3553); glEnable(2848);
        GL11.glShadeModel(7425);

        GL11.glBegin(9);
        glColor(topColor);
        for (int i = 0; i <= 90; i += 3) {
            double n = i * 0.017453292f;
            GL11.glVertex2d(x + radius + Math.sin(n) * radius * -1.0, y + radius + Math.cos(n) * radius * -1.0);
        }
        for (int j = 90; j <= 180; j += 3) {
            double n = j * 0.017453292f;
            GL11.glVertex2d(x + radius + Math.sin(n) * radius * -1.0, y2 - radius + Math.cos(n) * radius * -1.0);
        }
        glColor(bottomColor);
        for (int k = 0; k <= 90; k += 3) {
            double n = k * 0.017453292f;
            GL11.glVertex2d(x2 - radius + Math.sin(n) * radius, y2 - radius + Math.cos(n) * radius);
        }
        for (int l = 90; l <= 180; l += 3) {
            double n = l * 0.017453292f;
            GL11.glVertex2d(x2 - radius + Math.sin(n) * radius, y + radius + Math.cos(n) * radius);
        }
        GL11.glEnd();

        glEnable(3553); GL11.glDisable(3042); GL11.glDisable(2848);
        GL11.glPopAttrib(); GL11.glPopMatrix();
        GL11.glShadeModel(7424);
        GlStateManager.color(1f, 1f, 1f, 1f);
    }

    public static void drawRoundedGradientOutline(float x, float y, float x2, float y2, float radius, float lineWidth, int color1, int color2) {
        x *= 2.0f; y *= 2.0f; x2 *= 2.0f; y2 *= 2.0f;
        GL11.glPushMatrix();
        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
        GL11.glScaled(0.5, 0.5, 0.5);
